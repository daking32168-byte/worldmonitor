...
#### Coverage schema rollout status

The consumer-price v1 deploy-before-cron windows from [#6059](https://github.com/koala73/worldmonitor/issues/6059) closed on 2026-08-03. After the 14-day observation grace, their compiled per-market deadlines were removed from `api/health.js`; all eight coverage checks are now strict on every production sweep. A missing coverage key is `EMPTY` (crit), while present-but-empty, stale, partial, or degraded coverage keeps its corresponding strict verdict. No sibling key is used to synthesize success.

The daily publisher still writes the durable, versioned `seed-activated:consumer-prices:coverage:v1:<market>` marker only after a real page attempt. That preserves rollout provenance and prevents a future schema revision from inheriting activation earned by v1. The manual fallback deliberately does not write the marker because its coverage artifact has a short TTL.

`ROLLOUT_PENDING` remains a bounded health protocol for a future separately reviewed schema rollout, but there are currently no active static rollout entries. Any future entry must publish its deadline, remain within one daily scrape/aggregate/publish window, and be pruned after its observation grace; the freshness monitor fails closed on missing, malformed, or expired deadlines.
