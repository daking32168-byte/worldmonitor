...
[#6059](https://github.com/koala73/worldmonitor/issues/6059) 的 consumer-price v1「部署先于定时任务」窗口已于 2026-08-03 关闭。经过 14 天观察期后，八个市场的编译截止时间已从 `api/health.js` 移除；所有覆盖率检查现在在每次生产扫描中都执行严格判定。覆盖率键缺失为 `EMPTY`（严重），已存在但为空、过期、部分或降级时则保留各自严格状态，绝不通过兄弟键合成成功。

每日发布器仍只会在真实尝试页面后写入持久、带版本的 `seed-activated:consumer-prices:coverage:v1:<market>` 标记，用于保留上线谱系，并防止未来 schema 版本继承 v1 的激活。短 TTL 的手动兜底产物不会写入该标记。

`ROLLOUT_PENDING` 仍作为未来「单独评审且有界」的 schema 上线协议保留，但当前静态上线注册表为空。未来条目必须在响应中发布截止时间、限制在一个每日抓取/聚合/发布周期内，并在观察期后删除；截止时间缺失、格式错误或已过期时，freshness monitor 一律按失败关闭处理。
